...
def drinksformatter(data):
    ...
    embed.add_field(name="Altername Drink:", value=data.get("strDrinkAlternate"), inline=False)
    ...
    embed.add_field(name="Instructions:",value=data.get("strInstructions") ,inline=False)
...
def multipledrinksformatter(data, page):
    ...
    embed=discord.Embed(title=f"{data.get('strDrink')}", description=f"Page No: {page}", color=discord.Color.random())
...
def ingredientformatter(data):
    ...
    page1.set_image(url=f"https://www.thecocktaildb.com/images/ingredients/{data.get('strIngredient')}.png")

    pages.append(page1)

    page2=discord.Embed(color=discord.Color.random())
    for i in list(pagify(text=(data.get("strDescription")).strip("\r"), delims=['\n'], page_length=1024)):
